...
const initializeDownload = function({videos,selectedResolution},store){
  return new Promise(function(resolve, reject) {
      async.eachSeries(videos,(item,cb) => {
...
const startDownload = function({id,downloadQueue,selectedResolution},store){
  return new Promise(function(resolve, reject) {
                  async.eachSeries(downloadQueue,(item,cb) => {
                        item.resolution = selectedResolution;
...
function _readyDownloadQueue(item,callback,store)
{
  ...
  const vstore = store.download_queue[course_id];
   ...
   try{
       ...
       var url = list_videos[resolution] ||  list_videos['Auto'] || list_videos['720'] || list_videos['360'];
       if(!url) {
...
function _download(dataObj,callback, store) {
    ...
    let current_index = _.findIndex(vstore.downloadQueue,(o) => o.id == dataObj.id);
    let bar;
    let fileUrl = dataObj.data_url,
      base_path = dataObj.output ? files.getCurrentDirectoryBase() + path.sep + dataObj.output : files.getCurrentDirectoryBase(),
